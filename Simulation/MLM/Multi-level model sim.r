...
write.csv(re_mean,"re.csv")
write.csv(fe_mean,"fe.csv")

write.csv(res_list,"full_res.csv",row.names = FALSE)

re_mat = (re_list %x% rep(1,1,100))
...
colnames(rmse_df) = c("mlm_4_b0", "mlm_4_b1", "mlm_4_b2")
write.csv(rmse_df,"mlm_4_mc.csv",row.names = FALSE)

apply(apply((fe_mat + re_mat - true_b_mat), c(1,2), mean),c(2),sd)
